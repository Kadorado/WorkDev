...
<form action={{route('education.store')}} method="POST"> 
    ...
    <div class="flex flex-row flex-wrap">
        <select id="career" name="career" required class="flex-1 block p-2 mx-2 mt-1 bg-white border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            @foreach ($education as $edu)
                <option value="{{ $edu }}">{{ $edu}}</option>
            @endforeach
            
        </select>
        <select id="level" name="level" required class="flex-1 block w-1/2 p-2 mx-2 mt-1 bg-white border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
		...
		<table class="flex flex-row flex-wrap w-full my-2 overflow-hidden rounded-lg sm:bg-white sm:shadow-lg">
                ...
                <tbody class="flex-1 sm:flex-none">
                    <tr class="flex flex-col mb-2 flex-no wrap sm:table-row sm:mb-0">
                        <td class="h-20 p-3 text-xs border border-grey-light lg:w-2/5 lg:text-xl sm:w-2/5 sm:text-xl hover:bg-gray-100">{{ $userEdu->nameEducation }}</td>
                        <td class="p-3 truncate border border-grey-light lg:w-2/5 sm:w-2/5 hover:bg-gray-100">{{ $userEdu->level }}</td>
...
<form action="{{ route('education.store') }}" method="POST" class="my-6">
    ...
    <div class="flex flex-row flex-wrap">
        ...
        <select id="level" name="level" required class="flex-1 block p-2 mx-2 mt-1 bg-white border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            @foreach ($level as $lev)
                <option value="{{ $lev }}">{{ $lev }}</option>
            @endforeach    
        </select>
        <button class="flex-0.5 px-4 py-2 m-auto mx-6 font-bold text-green-600 transition duration-300 ease-in-out transform bg-green-200 rounded-full shadow-lg lg:mx-0 focus:outline-none focus:shadow-outline hover:bg-green-500 hover:text-white hover:scale-105" type="submit"> Agregar</button>
    </div>
</form>
