...
<form action="/vacante/{{$vacante->id}}" method="POST">
    ...
    <div>
       <label for="">Viejas</label>
        <select>
            @foreach ($userTecno as $tecno)
            <option >{{ $tecno->tecno}}</option>
            @endforeach
        </select>
    </div>
    ...
    <select x-cloak id="select">
        @foreach ($tecnologies as $tecnoall)
        <option value={{ $tecnoall->id}}>{{ $tecnoall->tecno}}</option>
        @endforeach
    </select>
    <div x-data="dropdown()" x-init="loadOptions()" class="w-full md:w-1/2 flex flex-col  h-64 mx-auto">
