...
@section('content')
    ...
    <section class="p-2 bg-white h-30 mt-5">
        <form class="container   flex justify-center py-8 mx-auto" action="/ofert/filterByCurrency" id="searchForm_currency">
            <select name="currency" id="currency_id"  class="shadow-lg w-1/3 h-12 px-3 mx-4 mb-8 rounded  text-s bg-white">
                <div class="w-full">
                    <option value="USD" class="relative flex items-center w-full p-2 pl-2 border-l-2 border">USD
                    </option>
                    <option value="COP" class="relative flex items-center w-full p-2 pl-2 border-l-2 border">COP
                    </option>
                    <option value="EUR" class="relative flex items-center w-full p-2 pl-2 border-l-2 border">EUR
                    </option>
                </div>
            </select>
    ...
    @else
        @foreach ($jobsDevs as $jobDev)
            ...
            <a href="{{ route('jobdetail', [$jobDev->Title, $jobDev->id]) }}">
                <div class="mb-2 border border-white bg-blue-400 rounded-lg shadow lg:flex">
                    <div class="block h-full py-1 bg-blue-400 rounded-lg shadow-inner lg:w-2/12">
                    ...
                    <div class="w-full px-1 py-5 tracking-wide bg-blue-400 lg:w-11/12 xl:w-full lg:px-2 lg:py-2">
                        <div class="flex flex-row justify-center lg:justify-start">
                            <div class="px-2 text-sm font-medium text-center text-white lg:text-left">
                                <i class="far fa-clock"></i>
                                {{ \Carbon\Carbon::parse($jobDev->created_at)->diffForHumans() }}
                            </div>
                            <div class="px-2 text-sm font-medium font-bold text-center text-white lg:text-left">
